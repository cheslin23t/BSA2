        ...
        # ensure password was submitted
            ...
            for user in db["users"]:
                ...
                if user["username"] == username and user[
                    ...
                    None
            if handled == False:
                return apology("Login not found in database.", "login.html")
...
def register():
    ...
    if request.method == "POST":
        ...
        password = ""
        # ensure username was submitted
        if not request.form.get("rootpassword"):
            return apology("Please enter the rootpassword.", "register.html")
        elif makeHash(
                request.form.get("rootpassword")
        ) != "e07016289eba7bc7487ce84d65ae7d87a3945f16f363d85904639f0aad0a71f7":

            print(request.form.get('password'))
            return apology(
                "Rootpassword is incorrect. Trust me, brute force will take you a thousand years to get in.",
                "register.html")
        elif not request.form.get("username"):
            return apology("Please enter an username.", "register.html")

        # ensure password was submitted
        elif not request.form.get("password"):
            return apology("Please enter a password.", "register.html")
...
def trip_rsvp():
    ...
    readableTripNames = {
        'irv_woods_0922': 'Irvington Woods Trip 2022-23',
        'irv_woods_0923': 'Irvington Woods Trip 2023-24'
    }
    allRsvp = []
    for trip in trips:
        try:
...
def isgoingformpost():
    ...
    scout = {'name': name, 'phone': phone, 'email': email}
    ...
    if trip in db:
        ...
        if rsvp == 'Yes':
            db[trip] = {'going': [scout], 'not_going': []}
        else:
            db[trip] = {'going': [], 'not_going': [scout]}
